...
app.get('/api/local_member', (req, res) => {
    ...
    db.query('SELECT num, PassRemain, PassType, LeaseTicket, SnackTicket, `A-lease`, `B-lease`, name FROM member WHERE code = ?', [memberCode], (err, memberResults) => {
        if (err) {
        ...
        if (memberResults.length > 0) {
            ...
            // Fetch the dog names
            db.query('SELECT name FROM dog WHERE code = ?', [memberCode], (err, dogResults) => {
                if (err) {
                    console.error('Error fetching dog names:', err);
                    res.status(500).json({ error: 'Error fetching dog names' });
                    return;
                }

                const dogNames = dogResults.map(dog => dog.name).join(', ');
                const visitLog = `Member: ${member.name}, Code: ${memberCode}, Dogs: ${dogNames}, Date: ${today}\n`;
                ...
                fs.readFile(path.join(logDirectory, 'visit.txt'), 'utf8', (err, data) => {
                    ...
                    const logExists = data && data.includes(`Code: ${memberCode}`) && data.includes(`Date: ${today}`);
...
app.post('/api/update_lease_a', (req, res) => {
    const { memberCode, count } = req.body;

    db.query('UPDATE member SET `A-lease` = `A-lease` + ? WHERE code = ?', [count, memberCode], (err) => {
        if (err) {
            console.error('Error updating lease A count:', err);
            res.status(500).json({ error: 'Error updating lease A count' });
            return;
        }
...
app.post('/api/update_lease_b', (req, res) => {
    const { memberCode, count } = req.body;

    db.query('UPDATE member SET `B-lease` = `B-lease` + ? WHERE code = ?', [count, memberCode], (err) => {
        if (err) {
            console.error('Error updating lease B count:', err);
            res.status(500).json({ error: 'Error updating lease B count' });
            return;
        }
...
app.get('/api/member_dogs', (req, res) => {
    const memberCode = req.query.memberCode;

    db.query('SELECT name FROM dog WHERE code = ?', [memberCode], (err, results) => {
        if (err) {
            console.error('Error fetching dog names:', err);
            res.status(500).json({ error: 'Error fetching dog names' });
            return;
...
app.post('/api/update_visit_log', (req, res) => {
    const { memberCode, date, dogName } = req.body;

    const logFilePath = path.join(logDirectory, 'visit.txt');
    ...
    fs.readFile(logFilePath, 'utf8', (err, data) => {
        ...
        logEntries.forEach(entry => {
            if (entry.includes(`Code: ${memberCode}`) && entry.includes(`Date: ${date}`)) {
                // Remove the dog name from the log entry
                let updatedEntry = entry.replace(`, ${dogName}`, '');
                updatedEntry = updatedEntry.replace(`${dogName}, `, '');
                updatedEntry = updatedEntry.replace(`${dogName}`, '');

                // If the log entry has "Dogs: " with no names left, remove the "Dogs: " part
                if (updatedEntry.includes('Dogs: ,')) {
                    updatedEntry = updatedEntry.replace('Dogs: ,', 'Dogs:');
                } else if (updatedEntry.includes('Dogs:, ')) {
                    updatedEntry = updatedEntry.replace('Dogs:, ', 'Dogs:');
                } else if (updatedEntry.includes('Dogs: ')) {
                    const dogsPart = updatedEntry.split('Dogs: ')[1].trim();
                    if (dogsPart === '') {
                        updatedEntry = updatedEntry.split('Dogs: ')[0].trim();
                    }
                }

                if (!entryUpdated) {
                    updatedLogEntries.push(updatedEntry);
                    entryUpdated = true; // Ensure only one updated entry is kept
                }
            } else {
                updatedLogEntries.push(entry);
            }
        });

        // Write the updated logs back to the file
        fs.writeFile(logFilePath, updatedLogEntries.join('\n'), 'utf8', err => {
            if (err) {
                console.error('Error updating visit log:', err);
                res.status(500).json({ error: 'Error updating visit log' });
                return;
            }
...
app.get('/api/todays_visit', (req, res) => {
    const { memberCode, date } = req.query;
    ...
    fs.readFile(logFilePath, 'utf8', (err, data) => {
        ...
        // Find today's log entry for the member
        const logEntries = data.split('\n');
        const todayLog = logEntries.find(entry => entry.includes(`Code: ${memberCode}`) && entry.includes(`Date: ${date}`));

        if (todayLog) {
            const dogNamesMatch = todayLog.match(/Dogs: (.*?), Date:/);
            if (dogNamesMatch && dogNamesMatch[1]) {
                const dogNames = dogNamesMatch[1].split(', ');
...
app.get('/api/visit_log', (req, res) => {
    ...
    fs.readFile(logFilePath, 'utf8', (err, data) => {
        ...
        const logEntries = data.split('\n');
        const visitingDogs = logEntries
            .filter(entry => entry.includes(`Code: ${memberCode}`) && entry.includes(`Date: ${date}`))
            .map(entry => {
                const match = entry.match(/Dogs: (.+?), Date/);
                return match ? match[1].split(', ') : [];
            }).flat();

        console.log('Fetched visiting dogs:', visitingDogs);
        res.json({ dogs: visitingDogs });
    });
});

app.post('/api/add_visit_log', (req, res) => {
    const { memberCode, date, dogName } = req.body;
    ...
    fs.readFile(logFilePath, 'utf8', (err, data) => {
        ...
        let updatedLogEntries = [];
        let entryUpdated = false;

        logEntries.forEach(entry => {
            if (entry.includes(`Code: ${memberCode}`) && entry.includes(`Date: ${date}`)) {
                // Add the dog name back to the log entry
                let updatedEntry = entry.replace(/Dogs: (.+?), Date/, (match, p1) => {
                    return `Dogs: ${p1}, ${dogName}, Date`;
                });
        ...
        if (!entryUpdated) {
            ...
            const newLogEntry = `Member: ${memberCode}, Dogs: ${dogName}, Date: ${date}\n`;
            updatedLogEntries.push(newLogEntry);
        }
